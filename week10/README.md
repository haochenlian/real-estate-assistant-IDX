...
| Email drafts and replies, follow-up questions, errors | Sent as written (the Week 11 email agent already uses WhatsApp formatting) |
...
- **Diagnostics stay out of the chat.** OpenClaw's `exec` tool hands the agent stdout and stderr
  together, so anything printed to stderr would reach the user. The bridge writes diagnostics
  to `~/.idx-assistant/bridge.log` (readable only by this Mac user) instead.
- **The OpenClaw agent never sends email itself.** `SKILL.md` tells it to pass replies to an
  email draft ("send", "cancel", "only 2 and 4", "note: …") to the bridge and never to email
  anyone with another tool.
  Whether an email goes out is decided by the Week 11 approval gate.
...
| `test_whatsapp_bridge.py` | 21 tests: formatting for every agent, session save/restore across processes, private file permissions, failure paths, keeping diagnostics out of the chat, and the command line. |
...
```bash
# Tests (should print 21/21 passed) — no WhatsApp, database, or API key needed
