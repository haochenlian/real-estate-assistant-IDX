...
LOG_PATH = os.path.expanduser(os.getenv("IDX_LOG_FILE", "~/.idx-assistant/bridge.log"))
...
def open_log():
    """The diagnostics log, readable only by this Mac user."""
    os.makedirs(os.path.dirname(LOG_PATH), mode=0o700, exist_ok=True)
    return open(os.open(LOG_PATH, os.O_WRONLY | os.O_CREAT | os.O_APPEND, 0o600), "a", encoding="utf-8")
...
def main(argv: list[str] | None = None) -> None:
    ...
    # OpenClaw's exec tool hands the agent stdout and stderr together, so anything written
    # to stderr would end up in the user's chat. Diagnostics go to a log file instead.
    with open_log() as log, contextlib.redirect_stderr(log):
        print(f"--- {datetime.now().isoformat(timespec='seconds')} user={args.user}", file=log)
        reply = reply_to(message, args.user)
    print(reply)
