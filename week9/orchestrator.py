...
from market import market_summary, price_trend, format_market_summary   # noqa: E402  Week 5
...
def price_direction(trend: list[dict], today: str | None = None, window: int = 3) -> dict:
    """Compare the average close price of the latest `window` months with the
    `window` months before them.

    Months after the current month are dropped: the sold table contains some
    impossible future close dates (e.g. 2072) that would otherwise count as
    "the latest month".
    """
    current = today or date.today().strftime("%Y-%m")
    months = [m for m in trend
              if m.get("month") and m["month"] <= current and m.get("avg_price")]
    if len(months) < window * 2:
        return {"direction": "not enough data", "change_pct": None}

    recent, prior = months[-window:], months[-2 * window:-window]
    recent_avg = sum(float(m["avg_price"]) for m in recent) / window
    prior_avg = sum(float(m["avg_price"]) for m in prior) / window
    change = round((recent_avg - prior_avg) / prior_avg * 100, 1)
    ...
    return {
        ...
        "recent_months": f"{recent[0]['month']} to {recent[-1]['month']}",
        "prior_months": f"{prior[0]['month']} to {prior[-1]['month']}",
    }
...
def market_stats_agent(query: str, user_id: str) -> dict:
    ...
    trend = price_direction(price_trend(city))
    text = format_market_summary(city, summary)
    if trend["change_pct"] is not None:
        text += (f"\n  Price trend     : {trend['direction']} "
                 f"({trend['change_pct']:+.1f}%) -- avg ${trend['recent_avg']:,} "
                 f"in {trend['recent_months']} vs ${trend['prior_avg']:,} "
                 f"in {trend['prior_months']}")
