...
Agent registry (each wraps code from an earlier week):
    ...
    email      emailDraftAgent       Week 11 draft-then-approve email
...
for _week in ("week2", "week3", "week4", "week5", "week6", "week7", "week8", "week11"):
...
from email_agent import email_agent, handle_reply as email_reply        # noqa: E402  Week 11
from email_agent import has_pending_draft, is_draft_command             # noqa: E402
...
PRICE_BAND = 300_000
MAX_ROWS = 50            # handbook safety rule: no query returns more than 50 rows


def build_candidates_query(city: str, price: int, exclude_id: str, limit: int = MAX_ROWS):
    ...
    sql = f"""
        ...
        LIMIT %s
    """
    return sql, [city, exclude_id, price - PRICE_BAND, price + PRICE_BAND, price, min(limit, MAX_ROWS)]
...
AGENTS = {
    ...
    "knowledge": rag_agent,
    "email": email_agent,             # Week 11: drafts, and sends only after "send"
}
...
def orchestrate(query: str, user_id: str, llm=None, agents: dict | None = None) -> dict:
    ...
    """
    # Replies to a pending email draft ("send", "cancel", "only 2 and 4", "note: …") are
    # handled here, before the model sees them: whether an email goes out, and what it
    # says, must not depend on how a classifier reads a short reply.
    if is_draft_command(query) and has_pending_draft(user_id):
        part = run_agent("email", email_reply, query, user_id)
        return {"intent": "email", "agents": ["email"], "text": part["text"], "parts": [part]}
